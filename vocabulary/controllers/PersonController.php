...
class PersonController extends RESTController
{
    ...
    public function getAction($request) {
        ...
        if(isset($request->url_elements[2])) {
			...
			if(is_int($searchid)){
				$row = $this->db->get_row("select * from earthchem.person where person_num=$searchid");
				if($row->person_num){
						$num = $row->person_num;
        	...
        	if($_GET){
				...
				$rows = $this->db->get_results("select * from earthchem.person order by last_name, first_name");
				$data['resultcount']=count($rows);
				foreach($rows as $row){
