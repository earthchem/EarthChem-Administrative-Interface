...
class MaterialController extends RESTController
{
    ...
    public function getAction($request) {
        ...
        if(isset($request->url_elements[2])) {
			...
			if(is_int($searchid)){
				$row = $this->db->get_row("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Material' and taxonomic_classifier_num=$searchid");
        	...
        	if($_GET){
        		...
        		if($_GET['label']){
					...
					$rows = $this->db->get_results("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Material' and lower(taxonomic_classifier_name) like '%$label%';
				...
				$rows = $this->db->get_results("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Material' order by taxonomic_classifier_name");
				$data['resultcount']=count($rows);
				foreach($rows as $row){
