...
class GraintypeController extends RESTController
{
    ...
    public function getAction($request) {
        ...
        if(isset($request->url_elements[2])) {
			...
			if(is_int($searchid)){
				$row = $this->db->get_row("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Grain Type' and taxonomic_classifier_num=$searchid");
        	...
        	if($_GET){
        		...
        		if($_GET['label']){
					...
					$rows = $this->db->get_results("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Grain Type' and lower(taxonomic_classifier_common_name) like '%$label%';
				...
				$rows = $this->db->get_results("select * from earthchem.taxonomic_classifier where taxonomic_classifier_type_cv='Grain Type' order by taxonomic_classifier_common_name");
				$data['resultcount']=count($rows);
				foreach($rows as $row){
